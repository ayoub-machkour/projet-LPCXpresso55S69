...
void sys_out_mode(int mode)
{
    ...
    switch (mode) {
    case CC_OUTPUT:
        uart_puts(USART0, "\x1b[34m");  // Couleur par défaut
        break;
    case CC_WARN:
        uart_puts(USART0, "\x1b[33m"); // Jaune pour les avertissements
        break;
    case CC_ERROR:
        uart_puts(USART0, "\x1b[31m"); // Rouge pour les erreurs
        break;
    case CC_EDIT:
        uart_puts(USART0, "\x1b[32m");  // Couleur par défaut
        uart_puts(USART0, "> ");       // Prompt de commande standard
        break;
    case CC_FEDIT:
        uart_puts(USART0, "\x1b[0m");  // Couleur par défaut
        uart_puts(USART0, "$ ");       // Prompt pour fonction
        break;
    default:
        break;
    }
...
{
    uart_puts(USART0, "\x1b[2J"); // Effacer l'écran et positionner le curseur en haut à gauche
}
...
{
    uart_puts(USART0, "\x1b[1D"); // Déplacer le curseur d'1 position vers la gauche
}

void move_cr_cb(void)
/* move the text cursor right */
{
    uart_puts(USART0, "\x1b[1C"); // Déplacer le curseur d'1 position vers la droite
}

void cursor_on_cb(void)
/* switch cursor on */
{
    uart_puts(USART0, "\x1b[?25h"); // Rendre le curseur visible
}

void cursor_off_cb(void)
/* switch cursor off */
{
    uart_puts(USART0, "\x1b[?25l"); // Rendre le curseur invisible
}
...
{
    uart_puts(USART0, "\x1b[5~"); // Envoyer la séquence Page Up
}

void page_down_cb(void)
{
    uart_puts(USART0, "\x1b[6~"); // Envoyer la séquence Page Down
}
...
char *fs_cd(char *dir)
{
    ...
    if (!card_ready) return path[0];

    if (dir == NULL)
    {
        return path[0]; // Retourne le chemin actuel sans rien faire
    }
    else
    {
        res = f_chdir(dir); // Change de répertoire
        if (res == FR_OK)
        {
            // Si le changement a réussi, obtient et met à jour le chemin courant
            res = f_getcwd(path[0], sizeof(cur_path));
        }
    }

    return path[0]; // Retourne le chemin courant (qui peut avoir changé)
}
