...
float iir_calc(iir_filter_t *f, float x)
{
	...
	for (int i=0; i< f->num_stages; i++) {
		bq->w[0]=y-bq->a[1]*bq->w[1]-bq->a[2]*bq->w[2];
		y=bq->b[0]*bq->w[0]+bq->b[1]*bq->w[1]+bq->b[2]*bq->w[2];
		bq->w[2]=bq->w[1];bq-> w[1]=bq->w[0];
		bq++;
	}
	return y*f->gain;
}
...
void pcm_biquad(real *b, real *a, int r, int c, real *n)
{
	...
	filt.gain=*b; b+=3; a+=3;
	for (int i=0; i<r-1; i++) {
		filt.biquads[i].b[0]=(float)*b++;
		filt.biquads[i].b[1]=(float)*b++;
		filt.biquads[i].b[2]=(float)*b++;
		filt.biquads[i].a[0]=(float)*a++;
		filt.biquads[i].a[1]=(float)*a++;
		filt.biquads[i].a[2]=(float)*a++;
...
void pcm_biquad(real *b, real *a, int r, int c, real *n)
{
	...
	for (int i=0; i<r; i++) {
		state[i].param.v_n_1=0.0;
		state[i].param.v_n=0.0;
		state[i].param.b_0=*b++;
		state[i].param.b_1=*b++;
		state[i].param.b_2=*b++;
		a++;
